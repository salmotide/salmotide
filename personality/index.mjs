...
export default async function ({
    ...
    try {
        ...
        if ((!q["16personalities"]) || (!imports.metadata.plugins["16personalities"].enabled(enabled, {extras}))) 
            ...
            //Load inputs
        let {url, sections, scores} = imports
            .metadata
            .plugins["16personalities"]
            .inputs({data, account, q})
        if (!url) 
            throw {
                error : {
        ...
        console.debug(
            `metrics/compute/${login}/plugins > 16personalities > starting browser`
        )
        const browser = await imports
            .puppeteer
            .launch()
        console.debug(
            `metrics/compute/${login}/plugins > 16personalities > started ${await browser.version()}`
        )
        const page = await browser.newPage()
        console.debug(
            `metrics/compute/${login}/plugins > 16personalities > loading ${url}`
        )
        ...
        const raw = await page.evaluate(() => ({
            ...
            type: document
                .querySelector(".type__code")
                .innerText,
            personality: [...document.querySelectorAll(".personality-cards .sp-personality-card")].map(
                card => ({
                    category: card
        ...
        const traits = raw
            .traits
            .map(({category, value, score, text}) => ({
                category,
                value: `${value[0]}${value
                    .substring(1)
                    .toLocaleLowerCase()}`,
                score: scores
                    ? Number(score.replace("%", "")) / 100
                    : NaN,
                text: text
                    .split(".")
                    .slice(1)
